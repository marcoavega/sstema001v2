...
if ($action === 'list') {
    ...
    $stock     = isset($_GET['stock'])     && $_GET['stock']     !== '' ? $_GET['stock']         : null;
    $priceFrom = isset($_GET['priceFrom']) && $_GET['priceFrom'] !== '' ? (float)$_GET['priceFrom'] : null;
    $priceTo   = isset($_GET['priceTo'])   && $_GET['priceTo']   !== '' ? (float)$_GET['priceTo']   : null;
    ...
    // Monta cláusula WHERE si hay algo
    ...
    try {
       ...
       $stmt = $db->prepare("
    SELECT 
        product_id, product_code, ... , status
    FROM products
    $whereSQL
